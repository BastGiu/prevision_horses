...
def display_dataframe(data_url):
    ...
    df.drop(columns=["cl", "pl", "id", "coteprob","cotedirect","predi_placé", "bonne_pred", "numcourse"], inplace=True)
    # df.heure = df.heure.map(lambda x: str(x)[0:-3])
    df.rename(columns={'jour' : 'Date','hippo': 'Hippodrome', 'prixnom': 'Prix', 'cheval': 'Cheval','numero' : 'Numéro','heure':'Heure course'}, inplace=True)
    return df
